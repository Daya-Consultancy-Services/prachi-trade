...
export default function TileAndSanitaryPage() {
    ...
    useEffect(() => {
        Promise.all([
            ...
            // Find the category for tile or sanitary
            const tileSanitaryCategory = categories.find(
                (cat) =>
                    cat.name.toLowerCase().includes("tile") ||
                    cat.name.toLowerCase().includes("sanitary")
            );
            if (!tileSanitaryCategory) {
                setProducts([]);
                setLoading(false);
                return;
            }
            const tileSanitarySubcategories = subcategories.filter(
                (sub) => sub.category === tileSanitaryCategory._id
            );
            const allProducts = tileSanitarySubcategories.flatMap((sub) => sub.products || []);
